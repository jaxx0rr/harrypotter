...
public class Hotbar extends Screen {
    ...
    protected static final ResourceLocation GUI_ICONS_LOCATION = new ResourceLocation(HarryPotterMod.MODID, "textures/gui/spells.png");

    @SuppressWarnings("static-access")
    public static void renderHotbar(float p_93010_, GuiGraphics guiGraphics, Gui gui, Minecraft minecraft) {
        ...
        if (player != null) {
            ...
            player.getCapability(PlayerStatsProvider.PLAYER_STATS_CAPABILITY).ifPresent(h -> {
                if (h.getBattleTick() == 1) {
                    ...
                    blit(guiGraphics, WIDGETS_LOCATION, i - l, minecraft.getInstance().getWindow().getGuiScaledHeight() - 22, 0, 0, 182, 22);
                    blit(guiGraphics, WIDGETS_LOCATION, i - l - 1 + h.getSelectedHotbar() * 20, minecraft.getInstance().getWindow().getGuiScaledHeight() - 22 - 1, 0, 22, 24, 22);
                }
            });

            if (!itemstack.isEmpty()) {
                if (humanoidarm == HumanoidArm.LEFT) {
                    blit(guiGraphics, WIDGETS_LOCATION, i - l - 29, minecraft.getInstance().getWindow().getGuiScaledHeight() - 23, 24, 22, 29, 24);
                } else {
                    blit(guiGraphics, WIDGETS_LOCATION, i + l, minecraft.getInstance().getWindow().getGuiScaledHeight() - 23, 53, 22, 29, 24);
                }
            }
                        ...
                        if (hotbar != null) {
                            if (!hotbar[x].equals("0")) {
                                ...
                                if ((Integer.parseInt(hotbar[x]) - 1) * 16 >= 256) {
                                    blit(guiGraphics, GUI_ICONS_LOCATION, i - l + 3 + x * 20,
            ...
            //gui.setBlitOffset(j);
            //guiGraphics.pose().translate(0, 0, j);
            ...
            if (minecraft.options.attackIndicator().get() == AttackIndicatorStatus.HOTBAR) {
                ...
                if (f < 1.0F) {
                    ...
                    int i2 = (int) (f * 19.0F);
                    RenderSystem.setShaderColor(1.0F, 1.0F, 1.0F, 1.0F);
                    blit(guiGraphics, GUI_ICONS_LOCATION, l2, k2, 0, 94, 18, 18);
                    blit(guiGraphics, GUI_ICONS_LOCATION, l2, k2 + 18 - i2, 18, 112 - i2, 18, i2);
                }
            }
